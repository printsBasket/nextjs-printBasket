...
const AdminLayoutWrapper: React.FC<AdminLayoutWrapperProps> = ({ children }) => {
    ...
    useEffect(() => {
        if (pathname !== '/admin/login/' && (!userInfo || !userInfo.isAdmin)) {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const orderNotifs = recentOrders.map((o: any) => ({
    ...
    // Avoid hydration mismatch before client-only state is ready
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="hidden md:flex items-center gap-2 text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                            ...
                            <span className="text-xs font-semibold font-mono">
                                {currentTime ? `${currentTime.toLocaleDateString()} • ${currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : '--:--'}
                            </span>
                        </div>
                    </div>
