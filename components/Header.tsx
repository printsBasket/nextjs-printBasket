...
const Header = () => {
  ...
  const showUserInfo = isMounted && !!userInfo;
  const showCartCount = isMounted && cartItems.length > 0;
  const cartCount = isMounted ? cartItems.reduce((acc: number, item: any) => acc + item.qty, 0) : 0;
  ...
  const getLinkClass = (href: string) => {
    ...
    return `${navItem} ${isActive ? "text-black after:w-full" : "hover:text-[#2364EB]"}`;
  };
      ...
      <header className="w-full bg-white border-b">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-3 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-5">
          ...
          <Link href="/" aria-label="Go to homepage">
            <img src="/logo-bgremove-224.png" alt="Prints Basket Logo" width="224" height="54" className="block w-32 h-auto object-contain" style={{ contentVisibility: 'auto' }} />
          </Link>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex">
          <div className="bg-white w-3/4 max-w-xs h-full p-6 relative flex flex-col">
            ...
            <nav className="flex flex-col gap-4 mt-8 flex-1">
              ...
              <Link href="/shop/" onClick={() => setMobileMenuOpen(false)} className="text-gray-700 font-medium hover:text-blue-500">Shop</Link>
